...
if len(sys.argv) > 4:
  basalgabacond = float(sys.argv[4])
if len(sys.argv) > 5:
  basal_t_jitter_sd = float(sys.argv[5])
if len(sys.argv) > 6:
...
for icell in range(0,1):
  ...
  template_file = "models/L5PCtemplate_withsyns_withdistinctgaba.hoc"
  v0 = -80
  ca0 = 0.0001
...
""")
  ...
  L5PC.setparameters(0.0,0.0,NsynE,NsynI)
  objref preTrainListLocal
  objref rds1,rds2
  preTrainListLocal = new List()
  rds1 = new Random(rdSeed)
  rds1.uniform(0,1)
  ...
  L5PC.distributeGABASynGivenDist(\"dend\",0,282)
    for(i2=0;i2<(NsynI);i2+=1){
    {preTrainListLocal.append(new Vector())}
    {preTrainListLocal.o[preTrainListLocal.count()-1].append("""+str(T_stim-basal_t_jitter_sd)+"""+"""+str(2*basal_t_jitter_sd)+"""*rds1.repick())}
    }
...
st1.dur = 0
  """)
    ...
    h("""
    L5PC.setparameters("""+str(Econs[min(2,iiter)])+""","""+str(basalgabacond)+""",NsynE,NsynI)
    ...
    for(syni=0;syni<NsynI;syni+=1) { 
      L5PC.synlist.o[NsynE+syni].gGABAmax = """+str(basalgabacond)+"""
    }
""")
    h.init()
    h.run()
  ...
  file = open('ffthreshs/ffthreshs_basalgabacond'+str(basalgabacond)+'_unijitter'+str(basal_t_jitter_sd)+'_Ihcoeff'+str(Ihcoeff)+'_'+treename+str(dist1)+'-'+str(dist2)+'_seed'+str(myseed)+'.sav', 'wb')
