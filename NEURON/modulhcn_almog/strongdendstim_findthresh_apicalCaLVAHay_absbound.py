...
if len(sys.argv) > 3:
  LVAcond = float(sys.argv[3])
if len(sys.argv) > 4:
  LVAcoeff = float(sys.argv[4])
  print("LVAcoeff "+str(LVAcoeff)+" given")
if len(sys.argv) > 5:
  dists_txt = sys.argv[5]
  print("dists "+str(dists_txt)+" given")
dists = [int(x) for x in dists_txt.split('-')]
...
#h("""forall if(ismembrane("cah")) { pbar_cah = 0 }""") #on second thoughts, keep the Almog Ca channels in soma and axon
#h("""forall if(ismembrane("car")) { pbar_car = 0 }""") #on second thoughts, keep the Almog Ca channels in soma and axon
h("""forsec apical {
  pbar_car = 0
  insert Ca_LVAst_hay
  gCa_LVAstbar_Ca_LVAst_hay = """+str(LVAcond)+"""
  if (distance(1) > """+str(dists[0])+""" && distance(0) < """+str(dists[1])+""") {
    gCa_LVAstbar_Ca_LVAst_hay = """+str(LVAcond*LVAcoeff)+"""
  }
}""")
...
syn1.onset = 200 + """+str(BACdt)+""" 
  """)
    ...
    Vmaxs_tested.append(max([Vsoma[i] for i in range(0,len(times)) if times[i] > 750]))
    Vmaxdends_tested.append(max([Vdend[i] for i in range(0,len(times)) if times[i] > 750]))
    Is_tested.append(Is_this[min(iI,2)])

    if iI < 2:
      continue
    if max([Vsoma[i] for i in range(0,len(times)) if times[i] > 750]) > -10:
    ...
    file = open('strongdendthresh'+str(denddist)+'_Ihcoeff'+str(Ihcoeff)+'_apicalCaLVAHay_'+str(LVAcond)+'_'+str(LVAcoeff)+'_dists'+dists_txt+'_absbound.sav', 'wb')
