...
Ihmod_txt = '10.0,-10.0,700.0'
myseed = 1
dist1 = 685
dist2 = 885
treename = 'apic'
if len(sys.argv) > 1:
  Ihmod_txt = sys.argv[1]
if len(sys.argv) > 2:
...
Ihmods = [float(Ihmod_txt.split(',')[0]),float(Ihmod_txt.split(',')[1])]
Ihmod_watershed_dist = float(Ihmod_txt.split(',')[2])
...
""")
  ...
  print("""forall if(ismembrane("Ih")) { 
  if(distance(0.5) < """+str(Ihmod_watershed_dist)+""") {offma_Ih = offma_Ih + """+str(Ihmods[0])+""" } else {offma_Ih = offma_Ih + """+str(Ihmods[1])+""" }
   }""")
  h("""forall if(ismembrane("Ih")) { 
  if(distance(0.5) < """+str(Ihmod_watershed_dist)+""") {offma_Ih = offma_Ih + """+str(Ihmods[0])+""" } else {offma_Ih = offma_Ih + """+str(Ihmods[1])+""" }
   }""")
...
""")
  ...
  file = open('ffthreshs/ffthreshs_Ihmod2ways'+Ihmod_txt+'_'+treename+str(dist1)+'-'+str(dist2)+'_seed'+str(myseed)+'.sav', 'wb')
