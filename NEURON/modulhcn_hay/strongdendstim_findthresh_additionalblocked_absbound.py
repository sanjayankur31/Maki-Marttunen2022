...
Ihcoeff = 1.0
denddist = 620
additionalblocked_suffixes = ['CaDynamics_E2','Ca_HVA','Ca_LVAst','Ih','Im','K_Pst','K_Tst','NaTa_t','NaTs2_t','Nap_Et2','SK_E2','SKv3_1']
additionalblocked = 'gNonebar_None'
if len(sys.argv) > 1:
  Ihcoeff = float(sys.argv[1])
if len(sys.argv) > 2:
  denddist = float(sys.argv[2])
if len(sys.argv) > 3:
  additionalblocked = sys.argv[3]

isuffChosen = -1
for isuff in range(0,len(additionalblocked_suffixes)):
  if additionalblocked.find(additionalblocked_suffixes[isuff]) > -1:
    isuffChosen = isuff
    break
if isuffChosen == -1:
  print('Suffix not existent, no additional blockage performed!!!')
...
""")
  ...
  if isuffChosen > -1:
    h("""forall if(ismembrane(\""""+additionalblocked_suffixes[isuffChosen]+"""\")) { """+additionalblocked+""" = 0 } """)
...
""")
    ...
    try:
      ...
      print('Too large I or too small atol? Take this as a spike, but double-check')
      if iI < 2:
        continue
    ...
    file = open('strongdendthresh'+str(denddist)+'_'+additionalblocked+'_Ihcoeff'+str(Ihcoeff)+'_absbound.sav', 'wb')
